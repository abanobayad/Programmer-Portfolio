...
@section('content')
    <div class=" d-felx justify-content-between mb-2">
        <h4 style="display: inline-block">Certificates</h4>
        <a class="btn btn-inverse-success" style="float: right" href="{{ route('certificate.add') }}">Add New</a>
    </div>
            ...
            <tbody id="tableData">
                ...
                @foreach ($certs as $cert)
                    ...
                    <tr>
                        ...
                        <td>{{ $cert->title }}</td>
                        <td><img class="rounded-circle" src="{{Storage::disk('s3')->url($cert->image)  }}" alt="{{ $cert->name}}"></td>

                        <td>
                            <a href="{{ route('certificate.edit', $cert->id) }}" class="btn text-warning"><i
                                class="menu-icon mdi mdi-border-color"></i></a>
                        <a href="{{ route('certificate.delete', $cert->id) }}" class="btn text-danger"><i
                                class="menu-icon mdi mdi-delete-sweep"></i></a>
                        <a href="{{ route('certificate.show', $cert->id) }}" class="btn text-primary"><i
                ...
                <div class="d-flex justify-content-center">
                    {{ $certs->appends(request()->input())->links() }}
                </div>
            </tbody>
        </table>
    </div>
